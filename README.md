...
启动一个二进制，浏览器打开就能看到实时日志流和历史查询。零外部依赖。

```bash
cd tools/logweb && make

# 启动面板，指定数据库路径
./logweb --db ../../example/data/log.db --port 8080
# 浏览器打开 http://localhost:8080
```

功能：
- 实时日志流（SSE 推送，新日志自动显示）
- 历史查询（按等级/关键词/时间范围过滤）
- 统计卡片（总数、ERROR/WARN 计数）
